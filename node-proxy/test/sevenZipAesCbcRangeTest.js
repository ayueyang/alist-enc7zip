...
import {
  ...
  getSevenZipAesCbcPreviewPlaceholderGif,
  getSevenZipAesCbcPreviewProfile,
  getSevenZipAesCbcPreviewStartSecond,
  normalizeSevenZipAesCbcPreviewDurationSeconds,
  normalizeSevenZipAesCbcPreviewQuality,
  prepareSevenZipAesCbcPreviewThumb,
} from '../src/utils/sevenZipAesCbcPreview'
import { decodeFromFolder, decodeName, encodeFromFolder, getAListFileTypeByName, pathFindPasswd } from '../src/utils/commonUtil'
import { normalizeProxyCacheConfig } from '../src/config'
import {
...
} from '../src/utils/proxyCacheManager'
...
function getPreviewFfmpegExe() {
  const candidates = [
    process.env.FFMPEG_PATH,
    path.join(process.cwd(), 'bin', 'ffmpeg', 'bin', process.platform === 'win32' ? 'ffmpeg.exe' : 'ffmpeg'),
  ].filter(Boolean)
  return candidates.find((item) => fs.existsSync(item)) || null
}

function createPreviewMp4Sample() {
  const ffmpegPath = getPreviewFfmpegExe()
  if (!ffmpegPath) return null
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'alist-7z-aes-cbc-preview-mp4-'))
  ...
  return { tempDir, plain: fs.readFileSync(plainPath) }
}
...
async function assertAListPreviewMiddleware(archivePath, plain) {
  process.env.RUN_MODE = 'DEV'
  // eslint-disable-next-line no-undef
  const Koa = require('koa')
  // eslint-disable-next-line no-undef
  ...
  const sevenZipAesCbcInfo = await parseSevenZipAesCbcInfoFromFile(archivePath, password)
  const stat = fs.statSync(archivePath)
  const listDir = `/alist-preview-${Date.now()}`
  const packagePath = `${listDir}/movie.7z`
  await cacheExternalSevenZipAesCbcInfo(
    { path: packagePath, name: 'movie.7z', size: stat.size, is_dir: false },
    sevenZipAesCbcInfo,
    password
  )
  ...
  const app = new Koa()
  app.use(async (ctx, next) => {
    ctx.req.reqBody = JSON.stringify({ path: listDir })
    ctx.req.webdavConfig = {
      passwdList: [
        {
          enable: true,
        ...
        content: [
          {
            ...
            is_dir: false,
            size: stat.size,
            type: getAListFileTypeByName('movie.7z'),
          },
        ],
      },
    }
  ...
  try {
    const resp = await requestText(
      ...
      JSON.stringify({ path: listDir })
    )
    const result = JSON.parse(resp.body)
    assert.strictEqual(result.code, 200)
    assert.strictEqual(result.data.content.length, 1)
    const item = result.data.content[0]
    assert.strictEqual(item.name, 'video.mp4')
    assert.strictEqual(item.path, `${listDir}/video.mp4`)
    assert.strictEqual(item.type, getAListFileTypeByName('video.mp4'))
    assert.ok(String(item.thumb || '').startsWith('/7z-aes-cbc-preview/'))
    assert.ok(String(item.thumb || '').endsWith('.gif?quality=high&duration=6'))
  } finally {
    await new Promise((resolve) => server.close(resolve))
  }
}

async function assertSevenZipAesCbcPreviewUtility() {
  assert.strictEqual(normalizeSevenZipAesCbcPreviewQuality('low'), 'low')
  assert.strictEqual(normalizeSevenZipAesCbcPreviewQuality('medium'), 'medium')
  assert.strictEqual(normalizeSevenZipAesCbcPreviewQuality('high'), 'high')
  assert.strictEqual(normalizeSevenZipAesCbcPreviewQuality('bad'), 'high')
  assert.strictEqual(normalizeSevenZipAesCbcPreviewDurationSeconds(3), 3)
  assert.strictEqual(normalizeSevenZipAesCbcPreviewDurationSeconds('6'), 6)
  assert.strictEqual(normalizeSevenZipAesCbcPreviewDurationSeconds(9), 9)
  assert.strictEqual(normalizeSevenZipAesCbcPreviewDurationSeconds('bad'), 6)
  assert.strictEqual(getSevenZipAesCbcPreviewStartSecond(0, 6), 0)
  assert.strictEqual(getSevenZipAesCbcPreviewStartSecond(5, 6), 0)
  assert.strictEqual(getSevenZipAesCbcPreviewStartSecond(20, 6), 7)
  assert.strictEqual(getSevenZipAesCbcPreviewStartSecond(101, 9), 46)
  assert.ok(getSevenZipAesCbcPreviewProfile('high').fps > getSevenZipAesCbcPreviewProfile('low').fps)

  const disabledThumb = await prepareSevenZipAesCbcPreviewThumb(
    {
      path: '/preview/movie.mp4',
      name: 'movie.mp4',
      type: getAListFileTypeByName('movie.mp4'),
      is_dir: false,
      size: 123,
    },
    {
  ...
  assert.strictEqual(disabledThumb, null)

  const thumb = await prepareSevenZipAesCbcPreviewThumb(
    {
      path: '/preview/movie.mp4',
      name: 'movie.mp4',
      type: getAListFileTypeByName('movie.mp4'),
      ...
      size: 456,
      plainSize: 123,
      sevenZipAesCbcPackagePath: '/preview/movie.mp4.7z',
      sevenZipAesCbcVirtualName: 'movie.mp4',
      sevenZipAesCbcInfo: {
        encType: SEVEN_ZIP_AES_CBC_ENC_TYPE,
        totalSize: 456,
        plainSize: 123,
    ...
    },
    {
      ...
      password,
      sevenZipAesCbcPreview: true,
      sevenZipAesCbcPreviewQuality: 'low',
      sevenZipAesCbcPreviewDurationSeconds: 9,
    }
  )
  assert.ok(String(thumb).startsWith('/7z-aes-cbc-preview/'))
  assert.ok(String(thumb).endsWith('.gif?quality=low&duration=9'))

  const durationThumb = await prepareSevenZipAesCbcPreviewThumb(
    {
      path: '/preview/movie.mp4',
      name: 'movie.mp4',
      type: getAListFileTypeByName('movie.mp4'),
    ...
    },
    {
      ...
      sevenZipAesCbcPreview: true,
      sevenZipAesCbcPreviewQuality: 'low',
      sevenZipAesCbcPreviewDurationSeconds: 3,
    }
  )
  assert.notStrictEqual(String(durationThumb), String(thumb))

  const key = String(thumb).split('/7z-aes-cbc-preview/')[1].split('.gif')[0]
  const missingToolResult = await getSevenZipAesCbcPreviewGif(key, 'low', {
    baseUrl: 'http://127.0.0.1:1',
    durationSeconds: 9,
  })
  assert.strictEqual(missingToolResult.placeholder, true)
  assert.deepStrictEqual(missingToolResult.body, getSevenZipAesCbcPreviewPlaceholderGif())

  const invalidKeyResult = await getSevenZipAesCbcPreviewGif('not-a-valid-key', 'low', {
    baseUrl: 'http://127.0.0.1:1',
    durationSeconds: 9,
  })
  assert.strictEqual(invalidKeyResult.placeholder, true)
  assert.strictEqual(invalidKeyResult.reason, 'invalid-preview-key')
}

async function assertSevenZipAesCbcPreviewGifGeneration(archivePath, plain) {
  const previewSample = createPreviewMp4Sample()
  if (!previewSample) {
    console.log('skip sevenZipAesCbcPreviewGifGeneration: ffmpeg not found')
    return
  }
  const previewArchive = createSevenZipSample(previewSample.plain, true)
  archivePath = previewArchive.archivePath
  plain = previewSample.plain
  process.env.RUN_MODE = 'DEV'
  // eslint-disable-next-line no-undef
  const Koa = require('koa')
  // eslint-disable-next-line no-undef
  ...
  const sevenZipAesCbcInfo = await parseSevenZipAesCbcInfoFromFile(archivePath, password)
  const stat = fs.statSync(archivePath)
  const listDir = `/preview-gif-${Date.now()}`
  const packagePath = `${listDir}/movie.7z`
  await cacheExternalSevenZipAesCbcInfo(
    { path: packagePath, name: 'movie.7z', size: stat.size, is_dir: false },
    sevenZipAesCbcInfo,
  ...
  app.use(async (ctx, next) => {
    ...
    ctx.req.origin = ''
    ctx.req.selfHost = ctx.request.host
    ctx.req.serverAddr = `http://${ctx.request.host}`
    ctx.req.urlAddr = `http://${ctx.request.host}${ctx.req.url}`
    await next()
  })
  app.use(encNameRouter.routes()).use(encNameRouter.allowedMethods())
  app.use(async (ctx) => {
    const filePath = decodeURIComponent((ctx.req.url || ctx.path).split('?')[0]).replace(/^\/d\//, '/')
    if (ctx.method === 'POST' && ctx.path === '/api/fs/list') {
      ctx.req.reqBody = JSON.stringify({ path: listDir })
      ctx.body = {
        code: 200,
        data: {
    ...
    if (ctx.method === 'GET' && filePath === `${listDir}/movie.7z`) {
      const range = ctx.headers.range
      const request = { method: 'GET', headers: { ...ctx.headers }, url: '/video.mp4', sevenZipAesCbcVirtualName: 'video.mp4' }
      prepareSevenZipAesCbcDownloadRequest(request, sevenZipAesCbcInfo, range)
      const encrypted = readFileRange(
        archivePath,
        request.sevenZipAesCbcPackageRange.start,
        request.sevenZipAesCbcPackageRange.end
      )
      const body = decryptSevenZipAesCbcRange(
        encrypted,
        sevenZipAesCbcInfo,
        request.sevenZipAesCbcPlainRange.start,
        request.sevenZipAesCbcPlainRange.end,
      ...
      ctx.status = request.sevenZipAesCbcPlainRange.hasRange ? 206 : 200
      ctx.set('content-type', 'video/mp4')
      ctx.set('accept-ranges', 'bytes')
      ctx.set('content-length', String(body.length))
      if (request.sevenZipAesCbcPlainRange.hasRange) {
        ctx.set(
          'content-range',
          `bytes ${request.sevenZipAesCbcPlainRange.start}-${request.sevenZipAesCbcPlainRange.end}/${plain.length}`
        )
      }
      ctx.body = body
      return
  ...
  try {
    const thumb = await prepareSevenZipAesCbcPreviewThumb(
      {
        path: `${listDir}/video.mp4`,
        name: 'video.mp4',
        type: getAListFileTypeByName('video.mp4'),
        is_dir: false,
        size: plain.length,
        plainSize: plain.length,
        sevenZipAesCbcPackagePath: packagePath,
        sevenZipAesCbcVirtualName: 'video.mp4',
    ...
    const key = String(thumb).split('/7z-aes-cbc-preview/')[1].split('.gif')[0]
    const asyncResult = await getSevenZipAesCbcPreviewGif(key, 'low', { baseUrl: url, durationSeconds: 3 })
    assert.strictEqual(asyncResult.placeholder, true)
    assert.strictEqual(asyncResult.reason, 'preview-generating')
    const result = await getSevenZipAesCbcPreviewGif(key, 'low', { baseUrl: url, durationSeconds: 3, waitForReady: true })
    assert.strictEqual(result.placeholder, false, result.reason)
    assert.ok(result.body.length > getSevenZipAesCbcPreviewPlaceholderGif().length)
    assert.deepStrictEqual(result.body.subarray(0, 6), Buffer.from('GIF89a'))
  } finally {
    await new Promise((resolve) => server.close(resolve))
  }
}

async function assertAListManagedOuterNameMiddleware() {
  process.env.RUN_MODE = 'DEV'
  // eslint-disable-next-line no-undef
  const Koa = require('koa')
  // eslint-disable-next-line no-undef
  const encNameRouter = require('../src/encNameRouter').default
  const listDir = `/alist-managed-${Date.now()}`
  const packageName = getSevenZipAesCbcManagedPackageName(password, 'video.mp4')
  const packagePath = `${listDir}/${packageName}`
  const app = new Koa()
  app.use(async (ctx, next) => {
    ctx.req.reqBody = JSON.stringify({ path: listDir })
    ctx.req.webdavConfig = {
        ...
        content: [
          {
            ...
            is_dir: false,
            size: 12345,
            type: getAListFileTypeByName(packageName),
          },
        ],
      },
    }
  ...
  try {
    ...
    const result = JSON.parse(resp.body)
    assert.strictEqual(result.data.content[0].name, 'video.mp4')
    assert.strictEqual(result.data.content[0].path, `${listDir}/video.mp4`)
    assert.strictEqual(result.data.content[0].type, getAListFileTypeByName('video.mp4'))
    assert.strictEqual(result.data.content[0].sevenZipAesCbcPackageName, packageName)

    const cachedVirtual = await getSevenZipAesCbcCachedFileInfoByVirtualPath(`${listDir}/video.mp4`, password)
    assert.ok(cachedVirtual, 'managed outer-name list should cache virtual-to-package mapping')
    assert.strictEqual(cachedVirtual.sevenZipAesCbcPackageName, packageName)
    assert.strictEqual(cachedVirtual.sevenZipAesCbcPackagePath, packagePath)
    assert.strictEqual(cachedVirtual.sevenZipAesCbcVirtualName, 'video.mp4')
  } finally {
    await new Promise((resolve) => server.close(resolve))
  }
}
...
async function assertGeneratedUploadArchive(upload, plain, expectedPath) {
  ...
  assert.ok(expectedPath.endsWith('.7z'))
  assert.ok(!path.basename(expectedPath).includes('video.mp4'))
  assert.ok(!path.basename(expectedPath).includes('.7zac'))
...
async function assertProxyCacheProductization() {
  const normalized = normalizeProxyCacheConfig({
    gifCacheMaxSizeMb: 4096,
    gifPreviewJobs: 2,
    gifPreviewQueueLimit: 64,
    gifTimeoutHighSeconds: 120,
    enableGifDiskCache: false,
    redirectCacheTtlHours: -1,
  })
  assert.strictEqual(normalized.config.gifCacheMaxSizeMb, 4096)
  assert.strictEqual(normalized.config.gifPreviewJobs, 2)
  assert.strictEqual(normalized.config.gifPreviewQueueLimit, 64)
  assert.strictEqual(normalized.config.gifTimeoutHighSeconds, 120)
  assert.strictEqual(normalized.config.enableGifDiskCache, false)
  assert.strictEqual(normalized.config.redirectCacheTtlHours, 72)
  assert.ok(normalized.warnings.some((item) => item.includes('redirectCacheTtlHours')))

  const redirectData = buildRedirectCacheData({
    redirectUrl: 'http://127.0.0.1/file',
    fileSize: 123,
    virtualName: 'video.mp4',
    sourcePath: '/movie/video.mp4',
    isWebdav: false,
    passwdInfo: { password: 'must-not-persist' },
  })
  assert.strictEqual(JSON.stringify(redirectData).includes('must-not-persist'), false)
  assert.strictEqual(redirectData.version, 2)

  await levelDB.setExpire('proxy-cache-test-redirect', redirectData, 60)
  const status = await getProxyCacheStatus()
  assert.ok(status.redirect.current >= 1)
  await clearProxyCache('redirect')
  assert.strictEqual(await levelDB.getValue('proxy-cache-test-redirect'), null)

  const importLikeNormalize = normalizeProxyCacheConfig({ gifCacheMaxFiles: 321, gifWidth: 'bad' })
  assert.strictEqual(importLikeNormalize.config.gifCacheMaxFiles, 321)
  assert.strictEqual(importLikeNormalize.config.gifWidth, 180)

  const beforeConfig = { ...getProxyCacheConfig() }
  const saved = saveProxyCacheConfig({ ...beforeConfig, gifCacheMaxFiles: 333 })
  assert.strictEqual(saved.config.gifCacheMaxFiles, 333)
  assert.throws(
    () => importProxyCacheConfig({ proxyCache: { ...saved.config, gifWidth: 'bad' } }),
    /gifWidth must be an integer/
  )
  assert.throws(
    () => importProxyCacheConfig({ proxyCache: { ...saved.config, unexpected: 1 } }),
    /unknown fields/
  )
  const missingFieldConfig = { ...saved.config }
  delete missingFieldConfig.version
  assert.throws(
    () => importProxyCacheConfig({ proxyCache: missingFieldConfig }),
    /missing fields/
  )
  assert.strictEqual(getProxyCacheConfig().gifCacheMaxFiles, 333)
  assert.strictEqual(getProxyCacheConfig().gifWidth, saved.config.gifWidth)
  const imported = importProxyCacheConfig({ proxyCache: { ...saved.config, gifCacheMaxFiles: 444 } })
  assert.strictEqual(imported.config.gifCacheMaxFiles, 444)
  saveProxyCacheConfig(beforeConfig)
}

function assertSevenZipAesCbcManagedName() {
  const packageName = getSevenZipAesCbcManagedPackageName(password, 'video.mp4')
  assert.ok(packageName.endsWith('.7z'))
  assert.ok(packageName.length <= 240)
  assert.ok(!packageName.includes('video.mp4'))
  assert.ok(!packageName.includes('.7zac'))
  assert.strictEqual(decodeName(password, SEVEN_ZIP_AES_CBC_ENC_TYPE, packageName.slice(0, -3)), 'video.mp4.7zac')
  assert.strictEqual(getSevenZipAesCbcInternalName('video.mp4'), 'video.mp4.7zac')
  assert.strictEqual(getSevenZipAesCbcManagedVirtualName(password, packageName), 'video.mp4')

  const markerInsideName = getSevenZipAesCbcManagedPackageName(password, 'foo.7zac.mp4')
  assert.strictEqual(getSevenZipAesCbcManagedVirtualName(password, markerInsideName), 'foo.7zac.mp4')

  assert.strictEqual(getSevenZipAesCbcManagedVirtualName(password, 'movie.7z'), null)
  assert.strictEqual(isSevenZipAesCbcTypedVirtualName('movie'), false)
  assert.strictEqual(isSevenZipAesCbcTypedVirtualName('movie.mp4'), true)
  assert.throws(
    () => getSevenZipAesCbcManagedPackageName(password, `${'一'.repeat(60)}.mp4`),
    /7z AES-CBC package name is too long/
  )
}
...
async function assertAListUploadPackagesSevenZipAesCbc(plain) {
  ...
  try {
    ...
    const expectedPackageName = getSevenZipAesCbcManagedPackageName(password, path.basename(uploadPath))
    const expectedPackagePath = `${path.dirname(uploadPath)}/${expectedPackageName}`
    ...
    await assertGeneratedUploadArchive(uploads[0], plain, expectedPackagePath)
    await assertGeneratedUploadCache(expectedPackagePath, plain)
    await assertGeneratedUploadCache('/dav' + expectedPackagePath, plain)
  } finally {
    await new Promise((resolve) => server.close(resolve))
    await new Promise((resolve) => originServer.close(resolve))
  }
...
async function assertWebdavUploadPackagesSevenZipAesCbc(plain) {
  ...
  try {
    ...
    await assertGeneratedUploadArchive(uploads[0], plain, expectedPackagePath)
    await assertGeneratedUploadCache(expectedPackagePath, plain)
    await assertGeneratedUploadCache(expectedPackagePath.substring(4), plain)
    ...
    assert.ok(!propfindResp.body.includes(expectedPackageName))
    ...
    assert.deepStrictEqual(getResp.body, plain.subarray(17, 97))
    assert.ok(
      requests.some((item) => item.method === 'GET' && item.url === expectedPackagePath),
...
function assertFolderPasswordRouting() {
  const folderToken = encodeFromFolder(password, SEVEN_ZIP_AES_CBC_ENC_TYPE, 'folder-secret', 'rc4')
  const folderName = `shared_${folderToken}`
  const decoded = decodeFromFolder(password, SEVEN_ZIP_AES_CBC_ENC_TYPE, folderName)
  assert.deepStrictEqual(decoded, { folderEncType: 'rc4', folderPasswd: 'folder-secret' })
  ...
  const { passwdInfo, pathInfo } = pathFindPasswd(passwdList, `/share/${folderName}/movie.mp4`)
  assert.ok(pathInfo)
  assert.strictEqual(passwdInfo.encType, 'rc4')
  assert.strictEqual(passwdInfo.password, 'folder-secret')
}
...
async function main() {
  ...
  assertSevenZipAesCbcManagedName()
  ...
  await assertSevenZipAesCbcPreviewUtility()
  await assertSevenZipAesCbcPreviewGifGeneration(webdavArchivePath, plain)
  await assertAListEntryMiddleware(webdavArchivePath, plain)
  await assertAListPreviewMiddleware(webdavArchivePath, plain)
  await assertAListManagedOuterNameMiddleware()
  await assertWebdavPropfindMiddleware(webdavArchivePath, plain)
  assertFolderPasswordRouting()
  await assertSevenZipPlainPassthroughFileName()
  await assertZipInfoCachePolicy()
  await assertProxyCacheProductization()
