...
import { decodeName, encodeName, getAListFileTypeByName } from './commonUtil'
import levelDB from './levelDB'
import { getArchiveProbeNegativeCacheSeconds } from './proxyCacheManager'
import {
  buildSevenZipAesCbcInfo,
  isSevenZipAesCbcEncType,
  parseSevenZipAesCbcInfoFromRemote,
  SEVEN_ZIP_AES_CBC_ENC_TYPE,
  SEVEN_ZIP_AES_CBC_NAME_MARKER,
  SEVEN_ZIP_AES_CBC_PACKAGE_NAME_MAX_LENGTH,
...
function normalizeFileName(fileName = '') {
  return path.basename(String(fileName || 'payload.bin')) || 'payload.bin'
}

function stripSevenZipAesCbcArchiveExt(fileName = '') {
  return String(fileName || '').slice(0, -3)
}

export function getSevenZipAesCbcInternalName(fileName = '') {
  return normalizeFileName(fileName) + SEVEN_ZIP_AES_CBC_NAME_MARKER
}

export function getSevenZipAesCbcManagedPackageName(password, fileName = '') {
  const internalName = getSevenZipAesCbcInternalName(fileName)
  const packageName = `${encodeName(password, SEVEN_ZIP_AES_CBC_ENC_TYPE, internalName)}.7z`
  if (packageName.length > SEVEN_ZIP_AES_CBC_PACKAGE_NAME_MAX_LENGTH) {
    throw new Error(
      `7z AES-CBC package name is too long: ${packageName.length}/${SEVEN_ZIP_AES_CBC_PACKAGE_NAME_MAX_LENGTH}`
    )
  }
  return packageName
}

export function getSevenZipAesCbcManagedVirtualName(password, packageName = '') {
  const normalizedName = normalizeFileName(packageName)
  if (!isSevenZipAesCbcFileName(normalizedName)) return null
  const decodedName = decodeName(password, SEVEN_ZIP_AES_CBC_ENC_TYPE, stripSevenZipAesCbcArchiveExt(normalizedName))
  if (!decodedName || !decodedName.endsWith(SEVEN_ZIP_AES_CBC_NAME_MARKER)) return null
  return decodedName.slice(0, -SEVEN_ZIP_AES_CBC_NAME_MARKER.length)
}

export function isSevenZipAesCbcTypedVirtualName(fileName = '') {
  return getAListFileTypeByName(fileName) !== 0
}
...
export async function getSevenZipAesCbcCachedFileInfoByVirtualPath(filePath, password) {
  ...
  if (
    ...
    cachedFileInfo.sevenZipAesCbcPackageName &&
    isMatchedSevenZipAesCbcPassword(cachedFileInfo, password)
  ) {
    return cachedFileInfo
  }
  let packagePath
  try {
    packagePath = normalizePath(path.dirname(normalizedPath) + '/' + getSevenZipAesCbcManagedPackageName(password, path.basename(normalizedPath)))
  } catch (e) {
    return null
  }
  ...
  if (cachedPackageInfo && !cachedPackageInfo.is_dir) {
    return {
      ...cachedPackageInfo,
      externalSevenZipAesCbc: true,
      sevenZipAesCbcVirtualName: path.basename(normalizedPath),
      sevenZipAesCbcPackageName: path.basename(packagePath),
...
export async function cacheSevenZipAesCbcManagedNameFileInfo(fileInfo, virtualName, password) {
  const packagePath = normalizePath(fileInfo.sevenZipAesCbcPackagePath || fileInfo.path)
  const packageName = fileInfo.sevenZipAesCbcPackageName || fileInfo.name || path.basename(packagePath)
  const virtualPath = normalizePath(path.dirname(packagePath) + '/' + virtualName)
  const nextFileInfo = {
    ...fileInfo,
    path: virtualPath,
    name: virtualName,
    ...
    sevenZipAesCbcVirtualName: virtualName,
    sevenZipAesCbcPackageName: packageName,
    sevenZipAesCbcPackagePath: packagePath,
  }
...
export async function cacheExternalSevenZipAesCbcNegative(fileInfo, error, password) {
  await levelDB.setExpire(
    ...
    getArchiveProbeNegativeCacheSeconds()
  )
}
...
export function getSevenZipAesCbcProbeRuntimeStats() {
  return {
    queueLength: probeQueue.length,
